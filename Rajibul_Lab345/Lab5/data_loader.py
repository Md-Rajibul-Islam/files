...
def gen_list(path, directory):
    ...
    for r, d, f in os.walk(path):
        for file in f:
            if directory in r:
                list_data.append(os.path.join(r, file))
    return list_data


def shuffle_split(Img, Mask, split_percent):
    np.random.shuffle(Img)
    np.random.shuffle(Mask)

    index_img = int(len(Img) * split_percent / 100)  # length of image train
    index_mask = int(len(Mask) * split_percent / 100)  # Length of mask train

    Img_train, Img_validation = Img[:index_img], Img[index_img:]
    Mask_train, Mask_validation = Mask[:index_mask], Mask[index_mask:]

    return Img_train, Img_validation, Mask_train, Mask_validation
...
def read_mask_onehot_encoding(data_path, list_, img_h, img_w):
    data = []
    for i in range(len(list_)):
        image_name = list_[i]
        img = imread(os.path.join(data_path, image_name), as_gray=True)
