...
def get_unet(input_img, n_filters, kernel_size, dropout=0.1, batchnorm=True):
    # Contracting Path
...
def get_unet_multi_organs(input_img, n_filters, kernel_size, N, dropout=0.1, batchnorm=True):
    # Contracting Path
    input_img = Input(input_img)
    c1 = conv2d_block(input_img, n_filters * 1, kernel_size=kernel_size, batchnorm=batchnorm)
    p1 = MaxPooling2D((2, 2))(c1)
    ...
    outputs = Conv2D(N, (1, 1), activation='softmax')(c9)
    model = Model(inputs=input_img, outputs=outputs)
    #model.summary()
    return model
